			...
			if not cmds.listConnections(obj + '.v'):
				...
				if drivers[0] == 'No drivers.':
					...
					for driver in drivers:
						...
						# driver 可能為 [curve 控制器] 或是 [受 curve 控制器 控制的 utilities 節點]
						...
						if outNodeDict:
							if driver_sn in outNodeDict.keys():
								...
								driver_ctrl = cmds.listConnections(driver_ln, d= 0, p= 1)
								if driver_ctrl:
									for dc in driver_ctrl:
										driver_ctrl_sn = dc.split(':')[-1].split('.')[0]
										if driver_ctrl_sn in outNodeDict.keys():
											# Driver [未登錄於 outNodeDict] 但控制此 Drive 的控制器 [已登錄於 outNodeDict]
											logger.debug('Visibility driverCTRL in [outNodeDict] -> [' + dc + '] - [' + driver_ln + ']')
											driver_sn = driver_ctrl_sn
											driver_at = dc.split('.')[1]
											goingOut = True
											break
						if goingOut:
							if driver_at in outNodeDict[driver_sn]:
								# 這個 Driver 已登錄，為避免之後 import Visibility 衝突就略過
								if cmds.objectType(obj) == 'transform' and cmds.listRelatives(obj, s= 1):
									if cmds.objectType(cmds.listRelatives(obj, s= 1, f= 1)[0]) == 'mesh':
										# 此物件為 [有 DrivenKey] 的 [Mesh] 且 [在 Transform] 但 [已登錄於 outNodeDict]
										logger.debug('Visibility driver in [outNodeDict] -> [' + driver + '] - [' + obj.split('|')[-1] + ']')
										driverHasOutkey = True
										break
								if cmds.objectType(obj) == 'mesh':
									# 此物件為 [有 DrivenKey] 的 [Mesh] 且 [在 Shape] 但 [已登錄於 outNodeDict]
									logger.debug('Visibility driver in [outNodeDict] -> [' + driver + '] - [' + obj.split('|')[-1] + ']')
									driverHasOutkey = True
									break
						# 檢查這個 Driver 是否有 Key
					...
					if driverHasViskey:
						...
						if not cmds.getAttr(obj + '.v'):
							...
							if objShouldRemove:
								# 此物件為 [有 DrivenKey] 但 [沒有設 key] 且 [隱藏] 的 [Mesh]
								logger.debug('No visibility drivenKey -x [' + obj.split('|')[-1] + ']')
							else:
								# 此物件為 [有 DrivenKey] 但 [已登錄於 outNodeDict] 的 [Mesh]
								pass
						else:
							# 此物件為 [有 DrivenKey] 雖然 [沒有設 key] 但為 [顯示] 的 [Mesh]
							pass
